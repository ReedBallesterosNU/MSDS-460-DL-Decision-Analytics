...
#Adjacency constraint, requiring counties with more adjacencies to be clustered closer 

n_adj = np.sum(df_TN_adj,axis = 0)
for j in range(n_districts):
    for i in range(n_counties):
            if n_adj[i] <= 4:
                model += 2*assignment[i][j] <= lpSum(df_TN_adj.iloc[i][k]*assignment[k][j] for k in range(n_counties))
            elif n_adj[i] <= 6:
                model += 3*assignment[i][j] <= lpSum(df_TN_adj.iloc[i][k]*assignment[k][j] for k in range(n_counties))
            elif n_adj[i] <= 8:
                model += 4*assignment[i][j] <= lpSum(df_TN_adj.iloc[i][k]*assignment[k][j] for k in range(n_counties))
            else:
                model += 5*assignment[i][j] <= lpSum(df_TN_adj.iloc[i][k]*assignment[k][j] for k in range(n_counties))
